...
Доступ только по whitelist из двух ролей: USER_TELEGRAM_ID (пользуется ботом) и DEVELOPER_TELEGRAM_ID (получает алерты о сбоях, тоже может пользоваться). Чужие апдейты — молча игнорировать (не отвечать), факт логировать. Алерты разработчику (bot/alerts.py): необработанные ошибки и проблемы с ключами API.
